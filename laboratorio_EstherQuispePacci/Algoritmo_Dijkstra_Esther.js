...
class Graph {
    ...
    addEdge(srcLabel, destLabel, weight = 1) {
        ...
        if (!srcVertex || !destVertex) {
            throw new Error("Vértices no encontrados");
        }
    ...
    floydWarshall() {
        ...
        this.mostrarResultadoFloydWarshall(distanceMatrix, verticesArray);
    }

    dijkstra(startLabel) {
        const startVertex = this.vertexMap.get(startLabel);
        if (!startVertex) {
            throw new Error("Vértice de inicio no encontrado");
        }
        ...
        this.mostrarResultadoDijkstra(distances);
    }

    mostrarResultadoFloydWarshall(distanceMatrix, vertices) {
    ...
    mostrarResultadoDijkstra(distances) {
        console.log("Vértice\tDistancia desde el Origen");
        distances.forEach((distance, vertex) => {
            console.log(`${vertex}\t${distance === Infinity ? 'Infinito' : distance}`);
        });
    }
}
